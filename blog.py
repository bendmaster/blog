...
class Post(db.Model):
    ...
    def render(self):
        '''Converts new lines into breaks for easier readability'''
...
class PostPage(BlogHandler):
    def get(self, post_id):
        '''Renders a page showing the recently created blog post,
        or a 404 page if the post does not exist'''
...
class NewPost(BlogHandler):
    ...
    def post(self):
        ...
        if subject and content:
            p = Post(parent = blog_key(), subject = subject, content = content, created_by = user)
            p.put()
